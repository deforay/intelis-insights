...
- EVERY column in the SELECT list MUST have a human-friendly alias using AS, and the alias MUST be wrapped in BACKTICKS (`). Title case, spaces allowed, no underscores. Example: SELECT gd.geo_name AS \`Province\`, COUNT(*) AS \`VL Tests\` — not SELECT gd.geo_name, COUNT(*) AS vl_test_count, and not SELECT … AS "Province" (double quotes are STRINGS in default-mode MySQL, not identifiers). Backticks make the alias an unambiguous identifier and let you use reserved words like Month, Year, Date, Type, Group without conflicting with MySQL's parser.
